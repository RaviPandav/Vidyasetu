...
function CourseForm({ course, onSave, onClose }) {
  const initialForm = course ? {
    ...
    title: "", description: "", shortDescription: "", category: "Mathematics",
    level: "All Levels", language: "English", price: 0, discountPrice: 0,
    requirements: "", learningOutcomes: "", targetAudience: "",
  };
  const [form, setForm] = useState(initialForm);
  const [thumbnail, setThumbnail] = useState(null);
  const [previewUrl, setPreviewUrl] = useState(course?.thumbnail ? getAssetUrl(course.thumbnail) : "");
  ...
  const handleThumbnailChange = (file) => {
    ...
    if (previewUrl?.startsWith("blob:")) URL.revokeObjectURL(previewUrl);
    setPreviewUrl(file ? URL.createObjectURL(file) : course?.thumbnail ? getAssetUrl(course.thumbnail) : "");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        targetAudience: typeof form.targetAudience === "string" ? form.targetAudience.split("\n").filter(Boolean) : form.targetAudience,
      };
      const requestData = new FormData();
      Object.entries(payload).forEach(([key, value]) => {
        if (value === undefined || value === null) return;
        requestData.append(key, Array.isArray(value) ? JSON.stringify(value) : value);
      });
  ...
  const field = (label, key, type = "text", extra = {}) => (
    <div>
      ...
      {type === "select" ? (
        ...
        <textarea rows={3} className="input-field" value={form[key] || ""} onChange={(e) => setForm({ ...form, [key]: e.target.value })} placeholder={extra.placeholder} />
      ) : (
        <input type={type} className="input-field" value={form[key] || ""} onChange={(e) => setForm({ ...form, [key]: type === "number" ? +e.target.value : e.target.value })} placeholder={extra.placeholder} />
      )}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b flex items-center justify-between">
          <h2 className="font-heading text-xl font-bold">{course ? "Edit Course" : "Create New Course"}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="w-full sm:w-40 aspect-video rounded-xl bg-primary-50 border border-gray-200 overflow-hidden flex items-center justify-center text-3xl">
                {previewUrl ? (
                  <img src={previewUrl} alt="Course preview" className="h-full w-full object-cover" />
                ) : (
                  "📚"
                )}
              </div>
              <div className="flex-1">
                ...
                />
                <p className="text-xs text-gray-500 mt-1">JPG, PNG, or WebP image upload karein.</p>
              </div>
            </div>
          </div>
          {field("Course Title *", "title", "text", { placeholder: "e.g. Complete Mathematics for Class 12" })}
          {field("Short Description", "shortDescription", "textarea", { placeholder: "Brief summary (max 300 chars)" })}
          ...
          <div className="grid grid-cols-2 gap-4">
            {field("Language", "language", "select", { options: ["English","Gujarati","Hindi","Both"] })}
            {field("Price (₹)", "price", "number")}
          </div>
          {field("Discount Price (₹)", "discountPrice", "number")}
          {field("Requirements (one per line)", "requirements", "textarea", { placeholder: "Basic mathematics knowledge\nAccess to internet" })}
          {field("Learning Outcomes (one per line)", "learningOutcomes", "textarea", { placeholder: "Understand calculus\nSolve integration problems" })}
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={loading} className="btn-primary flex-1">{loading ? "Saving..." : "Save Course"}</button>
...
function LectureModal({ course, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!targetSectionId) {
        const sectionRes = await teacherService.addSection(course._id, { title: newSectionTitle });
      ...
      toast.error(err.response?.data?.message || "Failed to add lecture video.");
    }
    setLoading(false);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">New Section Title</label>
            <input
              type="text"
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Upload a video file to attach to this lecture.</p>
          </div>

          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={loading} className="btn-primary flex-1">{loading ? "Saving..." : "Add Video"}</button>
            <button type="button" onClick={onClose} className="btn-ghost">Cancel</button>
          </div>
        </form>
      </div>
  ...
  return (
    <div className="space-y-6">
      {(showForm || editCourse) && (
        <CourseForm
          course={editCourse}
          onSave={() => { setShowForm(false); setEditCourse(null); fetchCourses(); }}
      ...
      {showLectureModal && lectureCourse && (
        <LectureModal
          ...
          onClose={() => { setShowLectureModal(false); setLectureCourse(null); }}
        />
      )}
          ...
          {courses.map((course) => (
            <div key={course._id} className="card flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-heading font-bold text-gray-900 truncate">{course.title}</h3>
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  ...
                  {course.isApproved ? <span className="badge-success text-xs">Approved</span> : <span className="badge-warning text-xs">Pending Approval</span>}
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={() => { setLectureCourse(course); setShowLectureModal(true); }}
                  className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200"
                >
                  <VideoCameraIcon className="w-4 h-4 inline-block mr-1" /> Add Video
                </button>
                <button
                ...
                <button onClick={() => setEditCourse(course)} className="btn-ghost p-2">
